...
SSM.STU = (function() {
	...
	function append2Dom(list) {
		...
		if (isNull(list)) {
			...
			var items = "";
			// 追加动态生成的组件
			var title = "<tr><th>选择</th><th>准考证号</th><th>姓名</th><th>性别</th><th>手机号码</th><th>地址</th></tr>";
			for (var i = 0; i < list.length; i++) {
				var tmp = "<tr><td><input type='checkbox' name='ids' value='"
						+ list[i].id + "' /></td><td>" + list[i].stuId
						+ "</td><td>" + list[i].stuName + "</td><td>"
						+ list[i].stuSex + "</td><td>"+list[i].stuTell
						+"</td><td>"+list[i].stuAddr+ "</td></tr>";
				items = items + tmp;
			}
	...
	function saveStu(id) { // 有id为更新，没有id为新增！
		...
		if (isNull(id)) {
			reqdata = '{"stuName":"' + $("#stuName").val() + '","stuSex":"'
					+ $("#stuSex").val() +'","stuTell":"'+$("#stuTell").val()
					+ '","stuAddr":"'+$("#stuAddr").val()
					+ '","stuId":"' + $("#stuId").val()
					+ '"}';
